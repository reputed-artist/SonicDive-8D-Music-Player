...
class AudioThread(QThread):
    ...
    def callback(self, outdata, frames, time, status):
        ...
        if self.seek_requested is not None:
            self.current_time = max(0, min(self.seek_requested, self.duration))
        ...
        # Update current time only if we didn't just seek
...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Create icons using Unicode symbols if image icons don't exist
        def create_icon_button(symbol, tooltip=""):
        ...
        # Create buttons with Unicode symbols
        self.btn_prev = create_icon_button("⏮", "Previous Track")
        self.btn_prev.clicked.connect(self.prev)
        
        self.btn_play = create_icon_button("▶", "Play/Pause")
        self.btn_play.clicked.connect(self.toggle_play)
        
        self.btn_next = create_icon_button("⏭", "Next Track")
    ...
    def seek_end(self):
        if self.audio.mono is not None and self.is_seeking:
    ...
    def update_position(self, position):
        if self.audio.mono is not None and not self.is_seeking:
            # Update progress bar
...
# ================= CREATE ICONS DIRECTORY WITH SIMPLE ICONS =================
def create_default_icons():
    ...
    # We'll use Unicode symbols in buttons instead of image files
    print("Using Unicode symbols for buttons - no image files needed!")
