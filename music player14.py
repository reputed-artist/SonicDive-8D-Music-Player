...
class Spectrum(QWidget):
    ...
    def paintEvent(self, e):
        ...
        #     # Draw central disk with gradient
        #     gradient = QLinearGradient(cx - disk_radius, cy - disk_radius, 
        #                               cx + disk_radius, cy + disk_radius)
        #     gradient.setColorAt(0, QColor(60, 60, 80, 220))
        #     gradient.setColorAt(1, QColor(40, 40, 60, 220))
            
        #     painter.setBrush(QBrush(gradient))
        #     painter.setPen(QPen(QColor(100, 100, 255, 180), 2))
        #     painter.drawEllipse(int(cx - disk_radius), int(cy - disk_radius), 
        #                        int(disk_radius * 2), int(disk_radius * 2))
            
        #     # Draw CD inner circle (like a CD hole)
        #     inner_radius = disk_radius * 0.15
        #     painter.setBrush(QBrush(QColor(20, 20, 30, 200)))
        #     painter.setPen(QPen(QColor(150, 150, 255, 100), 1))
        #     painter.drawEllipse(int(cx - inner_radius), int(cy - inner_radius), 
        #                        int(inner_radius * 2), int(inner_radius * 2))
            
        #     # Draw CD shine effect
        #     shine_radius = disk_radius * 0.9
        #     painter.setBrush(Qt.NoBrush)
        #     painter.setPen(QPen(QColor(200, 200, 255, 80), 1))
        #     painter.drawEllipse(int(cx - shine_radius), int(cy - shine_radius), 
        #                        int(shine_radius * 2), int(shine_radius * 2))
...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Effects (right side of the same line)
    ...
    def toggle_play(self):
        if self.audio.mono is None and self.files:
            ...
            if not self.audio.running:
                ...
                self.audio.running = True
                self.btn_play.setText("⏸")
                # Ensure the audio thread is running
                if not self.audio.isRunning():
                    self.audio.start()
            else:
                # Pause playback
