...
def extract_audio_thumbnail(file_path):
    """Extract thumbnail/album art from audio file"""
    ...
    try:
        if ext in ['.mp3', '.mp2', '.mp1']:
            ...
            # Try to find APIC frame
            for key in audio.keys():
                if key.startswith('APIC:'):
                    apic = audio[key]
                    if apic and hasattr(apic, 'data') and apic.data:
                        return BytesIO(apic.data)
            return None
        
        elif ext in ['.m4a', '.mp4', '.m4p', '.m4b', '.m4r', '.m4v']:
            # MP4 files
            audio = MP4(file_path)
            ...
            if audio.pictures:
                return BytesIO(audio.pictures[0].data)
            return None
...
class Spectrum(QWidget):
    ...
    def load_current_thumbnail(self):
        """Load thumbnail for the currently playing track"""
        # This will be called from MusicPlayer when track changes
        self.current_thumbnail = None
        self.update()

    def paintEvent(self, e):
        if self.mode == "Thumbnail":
            self.paint_thumbnail(e)
        else:
            self.paint_spectrum(e)

    def paint_thumbnail(self, e):
        """Paint thumbnail mode"""
        painter = QPainter(self)
        painter.setRenderHint(QPainter.Antialiasing)

        w, h = self.width(), self.height()
        
        # Draw dark background (same as other modes)
        painter.fillRect(0, 0, w, h, QColor(20, 20, 30))
        
        if self.current_thumbnail and not self.current_thumbnail.isNull():
            # Scale thumbnail to fit while maintaining aspect ratio
            thumb = self.current_thumbnail.scaled(
                w - 40, h - 40, 
                Qt.KeepAspectRatio, 
            ...
            # Draw border
            painter.setPen(QPen(QColor(100, 100, 150, 200), 2))
            painter.setBrush(Qt.NoBrush)
            painter.drawRect(x - 2, y - 2, thumb.width() + 4, thumb.height() + 4)
        else:
            ...
            painter.drawText(w // 2 - 20, h // 2 - 15, "🎵")
            
            # Draw message
            font = QFont("Segoe UI", 10)
            painter.setFont(font)
            painter.drawText(w // 2 - 100, h // 2 + 20, "No album art available")
            
            # Draw instruction
            painter.setPen(QColor(100, 100, 150))
            font = QFont("Segoe UI", 9)
            painter.setFont(font)
            painter.drawText(w // 2 - 120, h // 2 + 40, "Select a track to see album art")

    def paint_spectrum(self, e):
...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        visual_menu.addAction('Thumbnail').triggered.connect(lambda: self.spectrum.set_mode("Thumbnail"))
        ...
        self.spectrum_mode.addItems(["Bars", "Wave", "Circle", "Disk", "Thumbnail"])
    ...
    def play_selected(self, row=None):
        ...
        if 0 <= row < len(self.files):
            ...
            if self.audio.load(file_path):
                ...
                if self.spectrum.mode == "Thumbnail":
                    self.load_thumbnail_for_current_track()

    def load_thumbnail_for_current_track(self):
        """Load thumbnail for the currently playing track"""
        if self.current_file_index >= 0 and self.current_file_index < len(self.files):
            file_path = self.files[self.current_file_index]
            print(f"Loading thumbnail for: {os.path.basename(file_path)}")
            
            thumbnail_data = extract_audio_thumbnail(file_path)
            if thumbnail_data:
                try:
                    # Reset to beginning of BytesIO
                    thumbnail_data.seek(0)
                    pixmap = QPixmap()
                    if pixmap.loadFromData(thumbnail_data.read()):
                        print(f"Thumbnail loaded successfully")
                        self.spectrum.current_thumbnail = pixmap
                    else:
                        print("Failed to load thumbnail pixmap")
                        self.spectrum.current_thumbnail = None
                except Exception as e:
                    print(f"Error loading thumbnail: {e}")
                    self.spectrum.current_thumbnail = None
            else:
                print("No thumbnail data found in file")
                self.spectrum.current_thumbnail = None
