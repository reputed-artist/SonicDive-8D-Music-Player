...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Shuffle button with dynamic icons
        ...
        # Initialize with sequential icon
    ...
    def paint_shuffle_button(self, event):
        """Paint shuffle button with icon images"""
        ...
        if self.play_mode == "shuffle":
            # Try to load shuffle icon
            icon_path = "icons/shuffle.png"  # Your shuffle icon path
        elif self.play_mode == "repeat_one":
            # Try to load repeat_one icon
            icon_path = "icons/repeat_one.png"  # Your single repeat icon path
        else:  # sequential
            # Try to load sequential icon
            icon_path = "icons/sequential.png"  # Your sequential/repeat icon path
        ...
        if os.path.exists(icon_path):
            ...
            icon_size = QSize(24, 24)
            icon.paint(painter, 
                    (size.width() - icon_size.width()) // 2,
                    (size.height() - icon_size.height()) // 2,
                    icon_size.width(), icon_size.height())
        else:
            # Fallback to painted version if icon doesn't exist
            self.paint_shuffle_button_fallback(painter, size)
        
        painter.end()
        
    def paint_shuffle_button_fallback(self, painter, size):
        """Fallback painted version if icons don't exist"""
        ...
        # Draw fallback icons (same as before, just extracted to separate method)
        if self.play_mode == "shuffle":
            arrow_size = 8
    ...
    # Also fix the prev() function for shuffle mode:
    def prev(self):
