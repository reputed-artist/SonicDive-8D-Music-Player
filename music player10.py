...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.table.setColumnWidth(0, 230)  # Track column
        self.table.setColumnWidth(1, 60)   # Duration column
        ...
        self.table.setStyleSheet("""
            ...
            /* Blue disc for playing track */
            QTableWidget::item[playing="true"][column="0"]::before {
                content: "◉";
                color: #00b4d8;
                margin-right: 8px;
            }
    ...
    def load_folder(self):
        ...
        for f in os.listdir(folder):
            if f.lower().endswith((".mp3", ".wav", ".flac", ".ogg", ".m4a")):
                ...
                # Try to get duration (fast, just for display)
                duration = 0
                try:
                    y, sr = librosa.load(file_path, mono=True, sr=22050, duration=0.1)
                    # We'll get full duration when playing
                except:
                    pass
        ...
        if 0 <= row < len(self.files):
            ...
            for i in range(self.table.rowCount()):
                ...
                if track_item:
                    ...
                    # Ensure all tracks have gray disc
                    if text.startswith("●"):
                        # Already has gray disc, keep it
                        pass
                    elif text.startswith("◉"):  # Blue disc
                        # Replace blue disc with gray disc
                        track_item.setText("●" + text[1:])
            ...
            if self.audio.load(file_path):
                ...
                # Update duration display in table
                ...
                # Highlight playing track - Change disc to blue and set playing flag
                track_item = self.table.item(row, 0)
                if track_item:
                    current_text = track_item.text()
                    # Change gray disc (●) to blue disc (◉)
                    if current_text.startswith("●"):
                        track_item.setText("◉" + current_text[1:])
                    elif not current_text.startswith("◉"):
                        track_item.setText(f"◉ {current_text}")
