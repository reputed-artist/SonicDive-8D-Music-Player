...
class AudioThread(QThread):
    ...
    def callback(self, outdata, frames, time, status):
        ...
        if not self.running:
            ...
            # self.current_position += frames / self.sr
            # DON'T emit position_changed when paused
            # self.position_changed.emit(self.current_position)
            return
...
class MusicPlayer(QMainWindow):
    ...
    def init_ui(self):
        ...
        def create_transport_btn(icon_path=None, text="", tooltip=""):
            ...
            return btn
                ...
                # Shuffle button with custom painter
        self.btn_shuffle = QPushButton()
        self.btn_shuffle.setToolTip("Shuffle: Off")
        ...
        """)
        self.btn_shuffle.paintEvent = self.paint_shuffle_button
        ...
        transport_layout.addWidget(self.btn_shuffle)  # Add shuffle button
    ...
    def paint_shuffle_button(self, event):
        """Custom paint event for shuffle button"""
        painter = QPainter(self.btn_shuffle)
        painter.setRenderHint(QPainter.Antialiasing)
        
        # Set button color based on mode
        if self.play_mode == "shuffle":
            color = QColor(0, 180, 216)  # Cyan blue for shuffle
            painter.setPen(QPen(color, 2))
            painter.setBrush(Qt.NoBrush)
        elif self.play_mode == "repeat_one":
            color = QColor(255, 100, 100)  # Red for repeat one
            painter.setPen(QPen(color, 2))
            painter.setBrush(QBrush(color))
        else:  # sequential
            color = QColor(150, 150, 150)  # Gray for sequential
            painter.setPen(QPen(color, 1))
            painter.setBrush(Qt.NoBrush)
        
        size = self.btn_shuffle.size()
        ...
        if self.play_mode == "shuffle":
            # Draw two crossing arrows
            arrow_size = 8
            # First arrow (top-right to bottom-left)
            painter.drawLine(center_x - arrow_size, center_y - arrow_size, 
                           center_x + arrow_size, center_y + arrow_size)
            # Arrow head for first arrow
            painter.drawLine(center_x + arrow_size, center_y + arrow_size,
                           center_x + arrow_size - 3, center_y + arrow_size - 5)
            painter.drawLine(center_x + arrow_size, center_y + arrow_size,
                           center_x + arrow_size - 5, center_y + arrow_size - 3)
            ...
            # Draw a circle around the arrows
            painter.setBrush(Qt.NoBrush)
            painter.drawEllipse(center_x - radius, center_y - radius, 
                              radius * 2, radius * 2)
        
        elif self.play_mode == "repeat_one":
            # Draw repeat one icon (circle with arrow and number 1)
            painter.setBrush(QBrush(color))
            painter.drawEllipse(center_x - radius, center_y - radius, 
                              radius * 2, radius * 2)
            
            # Draw the number "1" in white
            painter.setPen(QPen(Qt.white, 2))
            painter.setFont(QFont("Arial", 10, QFont.Bold))
            painter.drawText(QRect(center_x - 6, center_y - 10, 12, 20), 
                           Qt.AlignCenter, "1")
            
            # Draw arrow shape around
            painter.setPen(QPen(color, 2))
            painter.setBrush(Qt.NoBrush)
            arrow_radius = radius + 3
            painter.drawEllipse(center_x - arrow_radius, center_y - arrow_radius,
                              arrow_radius * 2, arrow_radius * 2)
            
        else:  # sequential
            # Draw two arrows in a circle (sequential play)
            arrow_size = 6
            # First arrow (right arrow)
            painter.drawLine(center_x - arrow_size, center_y, 
                           center_x + arrow_size, center_y)
            painter.drawLine(center_x + arrow_size, center_y,
                           center_x + arrow_size - 3, center_y - 3)
            painter.drawLine(center_x + arrow_size, center_y,
                           center_x + arrow_size - 3, center_y + 3)
            
            # Second arrow (right arrow, slightly shifted)
            painter.drawLine(center_x - arrow_size + 2, center_y, 
                           center_x + arrow_size + 2, center_y)
            painter.drawLine(center_x + arrow_size + 2, center_y,
                           center_x + arrow_size - 1, center_y - 3)
            painter.drawLine(center_x + arrow_size + 2, center_y,
                           center_x + arrow_size - 1, center_y + 3)
    ...
    #     # Create shuffled list (excluding current playing track from first position)
    #     if self.current_file_index >= 0:
    #         remaining_indices = [i for i in range(len(self.files)) if i != self.current_file_index]
    #         random.shuffle(remaining_indices)
    #         self.shuffled_indices = [self.current_file_index] + remaining_indices
    ...
    def next(self):
        ...
        if self.play_mode == "shuffle":
            # Play next in shuffled order, or start over if at end
            if not self.shuffled_indices:
                self.create_shuffled_list()
            
            # Move to next in shuffled list
            self.current_shuffle_index = (self.current_shuffle_index + 1) % len(self.shuffled_indices)
            next_index = self.shuffled_indices[self.current_shuffle_index]
            
            print(f"Shuffle mode: Playing index {next_index} (position {self.current_shuffle_index} in shuffled list)")
        else:
            # Sequential play
    ...
    def prev(self):
        ...
        if self.play_mode == "shuffle":
            ...
            # This maintains the spirit of shuffle (not going back in a predetermined order)
            available_indices = [i for i in range(len(self.files)) if i != self.current_file_index]
            if not available_indices:  # Only one song in playlist
                prev_index = self.current_file_index
            else:
    ...
    def play_selected(self, row=None):
        """Play selected track and update shuffle state"""
        if row is None:
        ...
        if 0 <= row < len(self.files):
            ...
            # ... rest of existing play_selected code (with color highlighting, etc.)
    ...
    def clear_playlist(self):
        """Clear playlist and reset shuffle state"""
        self.files.clear()
        self.durations.clear()
